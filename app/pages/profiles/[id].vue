...
const { data, pending } = await useFetch<UserProfileDto[]>(`/api/users/${route.params.id}`)
...
<template>
  <section>
    <div class="intro">
      <p class="first">{{ (user && 'email' in user && profile?.email === user?.email) ? 'Here you can see how others see your profile' : `User with ID ${route.params.id}` }}</p>
    </div>
